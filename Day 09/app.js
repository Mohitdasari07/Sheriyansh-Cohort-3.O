...
function login() {
  ...
  if (!users[u] || users[u].pw !== p)
    return _err("loginErr", "Incorrect username or password.");
...
function register() {
  ...
  if (p.length < 4)
    return _err("registerErr", "Password must be at least 4 characters.");
...
function showPage(pg) {
  ...
  document.getElementById("nav-dashboard").className =
    "nav-item" + (pg === "dashboard" ? " active" : "");
  document.getElementById("nav-settings").className =
    "nav-item" + (pg === "settings" ? " active" : "");
...
function updateCards() {
  ...
  document.getElementById("statBalance").className =
    "stat-value" + (bal < 0 ? " red-val" : "");
}
...
function renderTable() {
  ...
  list.forEach((t) => {
    ...
    const dStr = new Date(t.date + "T00:00:00").toLocaleDateString("en-IN", {
      day: "2-digit",
      month: "short",
      year: "numeric",
    });
    ...
    tr.innerHTML =
      ...
      fmt(t.amt, sym) +
      "</td>" +
      '<td><button class="btn-del" onclick="deleteTx(' +
      t.id +
      ')">Delete</button></td>';
    body.appendChild(tr);
  });
      ...
      scales: {
        ...
        y: {
          ...
          ticks: {
            ...
            font: { size: 11 },
            callback: (v) =>
              (prefs.currency || "₹") + v.toLocaleString("en-IN"),
          },
        },
      },
    },
...
function updateSummary() {
  ...
  document.getElementById("savingsRate").className =
    "s-val " + (rate >= 0 ? "green-val" : "red-val");
...
function setType(t) {
  txType = t;
  document.getElementById("typInc").className =
    "type-btn" + (t === "income" ? " active" : "");
  document.getElementById("typExp").className =
    "type-btn" + (t === "expense" ? " active" : "");
}
