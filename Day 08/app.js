...
taskTitleInput.addEventListener("input", () => {
  if (formError.classList.contains("show") && taskTitleInput.value.trim())
    formError.classList.remove("show");
});
...
const refreshInspector = () => {
  ...
  document.getElementById("attrCategory").textContent =
    el.getAttribute("data-category") || "(removed)";

  document.getElementById("dsId").textContent = el.dataset.id;
  setStatusVal("dsStatus", el.dataset.status);
  document.getElementById("dsCategory").textContent =
    el.dataset.category || "(removed)";
};
...
const getSelectedEl = () =>
  selectedId
    ? taskList.querySelector(`.task-card[data-id="${selectedId}"]`)
    : null;
...
const refreshAttrView = () => {
  ...
  if (!el) {
    ...
    td.appendChild(
      document.createTextNode(
        "Select a task from the Tasks view to inspect it.",
      ),
    );
    tr.appendChild(td);
  ...
  const rows = [
    ...
    [
      ...
      el.getAttribute("data-category") || "null (removed)",
    ],
  ];
...
const logAttr = (message) => {
  ...
  while (attrConsole.children.length > 8)
    attrConsole.removeChild(attrConsole.firstChild);
};

btnHasAttr.addEventListener("click", () => {
  const el = getSelectedEl();
  if (!el) {
    logAttr("No task selected.");
    return;
  }
...
btnRemoveAttr.addEventListener("click", () => {
  ...
  logAttr(
    `removeAttribute('data-category') called. hasAttribute now → ${el.hasAttribute("data-category")}`,
  );
  refreshAttrView();
  refreshInspector();
});
...
btnRestoreAttr.addEventListener("click", () => {
  ...
  logAttr(
    `setAttribute('data-category', '${task ? task.category : "work"}') restored.`,
  );
  refreshAttrView();
  refreshInspector();
});
...
bubbleBtn.addEventListener("click", () => {
  ...
  clearPropConsole();
  logProp(
    'Mode: bubbling — click "Child Button" to see Child → Parent → Grandparent.',
  );
});
...
captureBtn.addEventListener("click", () => {
  ...
  clearPropConsole();
  logProp(
    'Mode: capturing — click "Child Button" to see Grandparent → Parent → Child.',
  );
});
...
childBtn.addEventListener(
  "click",
  () => {
    if (propMode === "bubble") logProp("Bubbling → Child Button");
  },
  false,
);
parentLayer.addEventListener(
  "click",
  () => {
    if (propMode === "bubble") logProp("Bubbling → Parent");
  },
  false,
);
grandparentLayer.addEventListener(
  "click",
  () => {
    if (propMode === "bubble") logProp("Bubbling → Grandparent");
  },
  false,
);
...
grandparentLayer.addEventListener(
  "click",
  () => {
    if (propMode === "capture") logProp("Capturing → Grandparent");
  },
  true,
);
parentLayer.addEventListener(
  "click",
  () => {
    if (propMode === "capture") logProp("Capturing → Parent");
  },
  true,
);
childBtn.addEventListener(
  "click",
  () => {
    if (propMode === "capture") logProp("Capturing → Child Button");
  },
  true,
);
